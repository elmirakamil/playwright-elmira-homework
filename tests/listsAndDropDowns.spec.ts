...
test('validate selected pet types from the list', async ({ page }) => {
    ...
    const petOwnerText = page.locator('.ownerFullName', { hasText: "George Franklin" })
    await expect(petOwner).toHaveText('George Franklin')
    ...
    const petTypesdropdown = page.locator('#type1')
    await expect(petTypesdropdown).toHaveValue('cat')
    ...
    for (const petTypeOption of petTypesOption) {
        ...
        await expect(petTypesdropdown).toHaveValue(petTypeOption)
    }
})
test('validate the pet type update', async ({ page }) => {
    ...
    const petTypesdropdown = page.locator('#type1')
    await expect(petTypesdropdown).toHaveValue('dog')
    ...
    await expect(petTypesdropdown).toHaveValue('bird')
    ...
    const rosyNewPetType = page.locator('text=Rosy').locator('..').locator('dd').last()
    await expect(rosyNewPetType).toHaveText('bird')
    ...
    page.getByRole('button', { name: "Update Pet" }).click()


    //Validate that Rosy's pet is reverted back to 'dog
    await expect(rosyNewPetType).toHaveText('dog')
})
